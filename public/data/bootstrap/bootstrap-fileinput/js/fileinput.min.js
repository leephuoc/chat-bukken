...
e("#"+b);var g=this.previewSettings.other||P.other,f=this.renderFileFooter(a.name,g.width),h=this.getPreviewTemplate("other"),k=b.slice(b.lastIndexOf("-")+1),m="";!0===c&&(m=" btn disabled");this.$preview.append("\n"+h.repl("{previewId}",b).repl("{frameClass}",m).repl("{fileindex}",k).repl("{caption}",this.slug(a.name)).repl("{width}",g.width).repl("{height}",g.height).repl("{type}",a.type).repl("{data}",d).repl("{footer}",f))}},previewFile:function(a,b,c,d,g){if(this.showPreview){var f=this.parseFileType(b),
e=this.slug(b.name),k=this.allowedPreviewTypes,m=this.allowedPreviewMimeTypes,l=this.getPreviewTemplate(f),p=A(f,this.previewSettings)?this.previewSettings[f]:P[f],q=parseInt(this.wrapTextLength,10),r=this.wrapIndicator,y=0<=k.indexOf(f),B=h(m)||!h(m)&&-1!==m.indexOf(b.type),m=this.renderFileFooter(e,p.width),L="",k=d.slice(d.lastIndexOf("-")+1);y&&B?("text"===f?(c=String(c.target.result).repl("&","&amp;").repl('"',"&quot;").repl("'","&#39;").repl("<","&lt;").repl(">","&gt;"),c.length>q&&(g="text-"+
M(),f=.75*window.innerHeight,L=this.getLayoutTemplate("modal").repl("{id}",g).repl("{title}",e).repl("{height}",f).repl("{body}",c),r=r.repl("{title}",e).repl("{dialog}","$('#"+g+"').modal('show')"),c=c.substring(0,q-1)+r),b=l.repl("{previewId}",d).repl("{caption}",e).repl("{frameClass}","").repl("{type}",b.type).repl("{width}",p.width).repl("{height}",p.height).repl("{data}",c).repl("{footer}",m).repl("{fileindex}",k)+L):b=l.repl("{previewId}",d).repl("{caption}",e).repl("{frameClass}","").repl("{type}",
b.type).repl("{data}",g).repl("{width}",p.width).repl("{height}",p.height).repl("{footer}",m).repl("{fileindex}",k),this.$preview.append("\n"+b),this.validateImage(a,d)):this.previewDefault(b,d)}},slugDefault:function(a){return h(a)?"":a.split(/(\\|\/)/g).pop().replace(/[^\w\u00C0-\u017F\-.\\\/ ]+/g,"")},getFileStack:function(a){return this.filestack.filter(function(b){return a?void 0!==b:void 0!==b&&null!==b})},readFiles:function(a){function b(e){h(d.attr("multiple"))&&(q=1);if(e>=q)c.isUploadable&&
0<c.filestack.length?c.raise("filebatchselected",[c.getFileStack()]):c.raise("filebatchselected",[a]),l.removeClass("loading"),k.html("");else{var w=p+"-"+(y+e),C,t=a[e],u=c.slug(t.name),D=(t.size||0)/1E3,x;C="";var F=N.createObjectURL(t),E=0,z,I=c.allowedFileTypes,G=h(I)?"":I.join(", "),J=c.allowedFileExtensions,H=h(J)?"":J.join(", ");h(J)||(C=new RegExp("\\.("+J.join("|")+")$","i"));D=D.toFixed(2);if(0<c.maxFileSize&&D>c.maxFileSize)z=c.msgSizeTooLarge.replace("{name}",u).replace("{size}",D).replace("{maxSize}",
c.maxFileSize),c.isError=B(z,t,w,e);else{if(!h(I)&&K(I)){for(D=0;D<I.length;D+=1)x=I[D],x=r[x],x=void 0!==x&&x(t.type,u),E+=h(x)?0:x.length;if(0===E){z=c.msgInvalidFileType.replace("{name}",u).replace("{types}",G);c.isError=B(z,t,w,e);return}}if(0===E&&!h(J)&&K(J)&&!h(C)&&(x=u.match(C),E+=h(x)?0:x.length,0===E)){z=c.msgInvalidFileExtension.replace("{name}",u).replace("{extensions}",H);c.isError=B(z,t,w,e);return}c.showPreview?(0<g.length&&void 0!==FileReader?(k.html(m.replace("{index}",e+1).replace("{files}",
q)),l.addClass("loading"),f.onerror=function(a){c.errorHandler(a,u)},f.onload=function(a){c.previewFile(e,t,a,w,F);c.initFileActions()},f.onloadend=function(){z=v.replace("{index}",e+1).replace("{files}",q).replace("{percent}",50).replace("{name}",u);setTimeout(function(){k.html(z);b(e+1);c.updateFileDetails(q)},100);c.raise("fileloaded",[t,w,e,f])},f.onprogress=function(a){a.lengthComputable&&(a=Math.ceil(a.loaded/a.total*100),z=v.replace("{index}",e+1).replace("{files}",q).replace("{percent}",a).replace("{name}",
u),setTimeout(function(){k.html(z)},100))},C=A("text",r)?r.text:Q.text,C(t.type,u)?f.readAsText(t,c.textEncoding):f.readAsArrayBuffer(t)):(c.previewDefault(t,w),setTimeout(function(){b(e+1);c.updateFileDetails(q)},100),c.raise("fileloaded",[t,w,e,f])),c.filestack.push(t)):(c.filestack.push(t),setTimeout(b(e+1),100),c.raise("fileloaded",[t,w,e,f]))}}}this.reader=new FileReader;var c=this,d=c.$element,g=c.$preview,f=c.reader,l=c.$previewContainer,k=c.$previewStatus,m=c.msgLoading,v=c.msgProgress,p=
c.previewInitId,q=a.length,r=c.fileTypeSettings,y=c.filestack.length,B=function(b,d,f,g){var k=e.extend(c.getOutData({},{},a),{id:f,index:g});g={id:f,index:g,file:d,files:a};c.previewDefault(d,f,!0);return c.isUploadable?c.showUploadError(b,k):c.showError(b,g)};b(0);c.updateFileDetails(q,!1)},updateFileDetails:function(a){var b=this.$element,c=this.getFileStack(),b=b.val()||c.length&&c[0].name||"",b=this.slug(b),c=this.isUploadable?c.length:a,d=l.count(this.id)+c,c=1<c?this.getMsgSelected(d):b;this.isError?
(this.$previewContainer.removeClass("loading"),this.$previewStatus.html(""),this.$captionContainer.find(".kv-caption-icon").hide()):this.showFileIcon();this.setCaption(c,this.isError);this.$container.removeClass("file-input-new file-input-ajax-new");1===arguments.length&&this.raise("fileselect",[a,b]);l.count(this.id)&&this.initPreviewDeletes()},change:function(a){var b=this.$element;if(!this.isUploadable&&h(b.val())&&this.fileInputCleared)this.fileInputCleared=!1;else{this.fileInputCleared=!1;var c,
d,g;d=this.$preview;var f=(c=1<arguments.length)?a.originalEvent.dataTransfer.files:b.get(0).files,b=h(b.attr("multiple"));g=0;var n,k=0,m=this.filestack.length,v=this.isUploadable;this.reader=null;this.resetUpload();this.hideFileIcon();this.isUploadable&&this.$container.find(".file-drop-zone ."+this.dropZoneTitleClass).remove();if(c)for(c=[];f[g];)n=f[g],n.type||0!==n.size%4096?c.push(n):k++,g++;else c=void 0===a.target.files?a.target&&a.target.value?[{name:a.target.value.replace(/^.+\\/,"")}]:[]:
a.target.files;h(c)||0===c.length?(v||this.clear(),this.showFolderError(k),this.raise("fileselectnone")):(this.resetErrors(),g=this.isUploadable?this.getFileStack().length+c.length:c.length,0<this.maxFileCount&&g>this.maxFileCount?(d=this.msgFilesTooMany.replace("{m}",this.maxFileCount).replace("{n}",g),b=e.extend(this.getOutData({},{},f),{id:null,index:null}),f={id:null,index:null,file:null,files:f},this.isError=this.isUploadable?this.showUploadError(d,b):this.showError(d,f),this.$captionContainer.find(".kv-caption-icon").hide(),
this.setCaption("",!0),this.setEllipsis(),this.$container.removeClass("file-input-new file-input-ajax-new")):(!v||b&&0<m?(this.hasInitialPreview()?(f=l.out(this.id),d.html(f.content),this.setCaption(f.caption),this.initPreviewDeletes()):this.clearPreview(),b&&0<m&&(this.filestack=[])):!v||0!==m||l.count(this.id)&&!this.overwriteInitial||(this.clearPreview(),this.filestack=[]),this.isPreviewable?this.readFiles(c):this.updateFileDetails(1),this.showFolderError(k)))}},validateImage:function(a,b){var c=
this,d=c.$preview,g,f,e,k,h=d.find("#"+b),l="Untitled",p=h.find("img");if(p.length)p.on("load",function(){f=h.width();e=d.width();f>e&&(p.css("width","100%"),h.css("width","97%"));k=p.closest(".file-preview-frame").find(".file-caption-name");k.length&&(k.width(p.width()),l=k.text(),k.attr("title",l));g={ind:a,id:b};c.checkDimensions(a,"Small",p,h,l,"Width",g);c.checkDimensions(a,"Small",p,h,l,"Height",g);c.checkDimensions(a,"Large",p,h,l,"Width",g);c.checkDimensions(a,"Large",p,h,l,"Height",g);c.raise("fileimageloaded",
[b]);N.revokeObjectURL(p.attr("src"))})},checkDimensions:function(a,b,c,d,e,f,l){var k=this[("Small"===b?"min":"max")+"Image"+f];!h(k)&&c.length&&(c=c[0],c="Width"===f?c.naturalWidth||c.width:c.naturalHeight||c.height,("Small"===b?c>=k:c<=k)||(b=this["msgImage"+f+b].replace("{name}",e).replace("{size}",k),this.showUploadError(b,l),this.setThumbStatus(d,"Error"),this.filestack[a]=null))},initCaption:function(){var a=this.initialCaption||"";if(this.overwriteInitial||h(a))return this.$caption.html(""),
!1;this.setCaption(a);return!0},setCaption:function(a,b){var c,d;if(b)c=e("<div>"+this.msgValidationError+"</div>").text(),d='<span class="'+this.msgValidationErrorClass+'">'+this.msgValidationErrorIcon+c+"</span>";else{if(h(a)||0===this.$caption.length)return;c=e("<div>"+a+"</div>").text();d=this.getLayoutTemplate("icon")+c}this.$caption.html(d);this.$caption.attr("title",c);this.$captionContainer.find(".file-caption-ellipsis").attr("title",c);this.setEllipsis()},initBrowse:function(a){this.$btnFile=
...
" fileinput-cancel fileinput-cancel-button")+'">'+this.cancelIcon+this.cancelLabel+"</button>":""},renderUpload:function(){var a=this.uploadClass+" kv-fileinput-upload fileinput-upload-button",b="",b="";if(!this.showUpload)return"";this.isDisabled&&(b=" disabled ");return b=!this.isUploadable||this.isDisabled?'<button type="submit" title="'+this.uploadTitle+'"class="'+a+'"'+b+">"+this.uploadIcon+this.uploadLabel+"</button>":'<a href="'+this.uploadUrl+'" title="'+this.uploadTitle+'" class="'+a+'"'+
b+">"+this.uploadIcon+this.uploadLabel+"</a>"}};e.fn.fileinput=function(a){if(window.File&&window.FileReader||F(9)){var b=Array.apply(null,arguments);b.shift();return this.each(function(){var c=e(this),d=c.data("fileinput"),g="object"===typeof a&&a,f=g.language||c.data("language")||"en";d||(d=e.extend({},e.fn.fileinput.defaults),"en"===f||h(e.fn.fileinputLocales[f])||(d=e.extend(d,e.fn.fileinputLocales[f])),d=new O(this,e.extend(d,g,c.data())),c.data("fileinput",d));"string"===typeof a&&d[a].apply(d,
b)})}};e.fn.fileinput.defaults={language:"en",showCaption:!0,showPreview:!0,showRemove:!0,showUpload:!0,showCancel:!0,showUploadedThumbs:!0,mainClass:"",previewClass:"",captionClass:"",mainTemplate:null,initialCaption:"",initialPreview:[],initialPreviewDelimiter:"*$$*",initialPreviewConfig:[],initialPreviewThumbTags:[],previewThumbTags:{},initialPreviewShowDelete:!0,deleteUrl:"",deleteExtraData:{},overwriteInitial:!0,layoutTemplates:R,previewTemplates:S,allowedPreviewTypes:H,allowedPreviewMimeTypes:null,
allowedFileTypes:null,allowedFileExtensions:null,customLayoutTags:{},customPreviewTags:{},previewSettings:P,fileTypeSettings:Q,previewFileIcon:'<i class="glyphicon glyphicon-file"></i>',browseIcon:'<i class="glyphicon glyphicon-folder-open"></i> &nbsp;',browseClass:"btn btn-primary",removeIcon:'<i class="glyphicon glyphicon-trash"></i> ',removeClass:"btn btn-default",cancelIcon:'<i class="glyphicon glyphicon-ban-circle"></i> ',cancelClass:"btn btn-default",uploadIcon:'<i class="glyphicon glyphicon-upload"></i> ',
uploadClass:"btn btn-default",uploadUrl:null,uploadAsync:!0,uploadExtraData:{},minImageWidth:null,minImageHeight:null,maxImageWidth:null,maxImageHeight:null,maxFileSize:0,minFileCount:0,maxFileCount:0,msgValidationErrorClass:"text-danger",msgValidationErrorIcon:'<i class="glyphicon glyphicon-exclamation-sign"></i> ',msgErrorClass:"file-error-message",progressClass:"progress-bar progress-bar-success progress-bar-striped active",progressCompleteClass:"progress-bar progress-bar-success",previewFileType:"image",
wrapTextLength:250,wrapIndicator:' <span class="wrap-indicator" title="{title}" onclick="{dialog}">[&hellip;]</span>',elCaptionContainer:null,elCaptionText:null,elPreviewContainer:null,elPreviewImage:null,elPreviewStatus:null,elErrorContainer:null,slugCallback:null,dropZoneEnabled:!0,dropZoneTitleClass:"file-drop-zone-title",fileActionSettings:{},otherActionButtons:"",textEncoding:"UTF-8",ajaxSettings:{},ajaxDeleteSettings:{},showAjaxErrorDetails:!0};e.fn.fileinputLocales.en={fileSingle:"file",filePlural:"files",
browseLabel:"Browse &hellip;",removeLabel:"Remove",removeTitle:"Clear selected files",cancelLabel:"Cancel",cancelTitle:"Abort ongoing upload",uploadLabel:"Upload",uploadTitle:"Upload selected files",msgSizeTooLarge:'File "{name}" (<b>{size} KB</b>) exceeds maximum allowed upload size of <b>{maxSize} KB</b>. Please retry your upload!',msgFilesTooLess:"You must select at least <b>{n}</b> {files} to upload. Please retry your upload!",msgFilesTooMany:"Number of files selected for upload <b>({n})</b> exceeds maximum allowed limit of <b>{m}</b>. Please retry your upload!",
msgFileNotFound:'File "{name}" not found!',msgFileSecured:'Security restrictions prevent reading the file "{name}".',msgFileNotReadable:'File "{name}" is not readable.',msgFilePreviewAborted:'File preview aborted for "{name}".',msgFilePreviewError:'An error occurred while reading the file "{name}".',msgInvalidFileType:'Invalid type for file "{name}". Only "{types}" files are supported.',msgInvalidFileExtension:'Invalid extension for file "{name}". Only "{extensions}" files are supported.',msgValidationError:"File Upload Error",
...
dropZoneTitle:"Drag & drop files here &hellip;"};e.extend(e.fn.fileinput.defaults,e.fn.fileinputLocales.en);e.fn.fileinput.Constructor=O;e(document).ready(function(){var a=e("input.file[type=file]");a.length&&a.fileinput()})})(window.jQuery);